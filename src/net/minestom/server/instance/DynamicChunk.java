...
import net.minestom.server.timer.ServerRunnable;
...
public class DynamicChunk extends Chunk {
    ...
    protected final HashMap<Integer,Data> blocksData = new HashMap<>();

    // Contains CustomBlocks' block index which are updatable
    protected final HashSet<Integer> updatableBlocks = new HashSet<>();
    // (block index)/(last update in ms)
    protected final HashMap<Integer,Long> updatableBlocksLastUpdate = new HashMap<>();

    // Block entities
    protected final HashSet<Integer> blockEntities = new HashSet<>();

    private long lastChangeTime;

    public DynamicChunk(Biome[] biomes, int chunkX, int chunkZ,
                        PaletteStorage blockPalette, PaletteStorage customBlockPalette) {
    ...
    public void tick(long time, Instance instance) {
        ...
        final Iterator<Integer> iterator = new ArrayList<>(updatableBlocks).iterator();
    ...
    public void readChunk(BinaryReader reader, ChunkCallback callback) {
        ...
        new ServerRunnable() {
            ...
            synchronized (this) {
                ...
                HashMap<String,Short> typeToIndexMap = null;
                ...
                for (int i = 0; i < BIOME_COUNT; i++) {
                    ...
                    DynamicChunk.this.biomes[i] = BIOME_MANAGER.getById(id);
                }
                ...
                // Finished reading
                OptionalCallback.execute(callback, DynamicChunk.this);
            }
        }}.runTask();
    }
    ...
    protected ChunkDataPacket createFreshPacket() {
        ...
        fullDataPacket.blockEntities = new ArrayList<>();
        fullDataPacket.blocksData = new HashMap<>();
    ...
    private short getBlockAt(PaletteStorage paletteStorage, int x, int y, int z) {
        return paletteStorage.getBlockAt(x, y, z);
    }

    private void setBlockAt(PaletteStorage paletteStorage, int x, int y, int z, short blockId) {
