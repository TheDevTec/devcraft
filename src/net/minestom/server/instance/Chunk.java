...
public abstract class Chunk implements Viewable, DataContainer {
    ...
    public Chunk(Biome[] biomes, int chunkX, int chunkZ, boolean shouldGenerate) {
     ...
     */
    public synchronized void sendChunk(Player player) {
     ...
     */
    public synchronized void sendChunkUpdate(Player player) {
        final PlayerConnection playerConnection = player.getPlayerConnection();
        playerConnection.sendPacket(getFreshFullDataPacket());
    }
     ...
     */
    public synchronized void sendChunkSectionUpdate(int section, Player player) {
