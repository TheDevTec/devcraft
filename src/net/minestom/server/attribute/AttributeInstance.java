...
public class AttributeInstance {
    ...
    public AttributeInstance(Attribute attribute, Consumer<AttributeInstance> listener) {
    ...
    protected void refreshCachedValue() {
        ...
        for (AttributeModifier modifier : modifiers.stream().filter(mod -> mod.getOperation() == AttributeOperation.ADDITION).toArray(AttributeModifier[]::new)) {
            base += modifier.getAmount();
        }

        float result = base;

        for (AttributeModifier modifier : modifiers.stream().filter(mod -> mod.getOperation() == AttributeOperation.MULTIPLY_BASE).toArray(AttributeModifier[]::new)) {
            result += (base * modifier.getAmount());
        }
        for (AttributeModifier modifier : modifiers.stream().filter(mod -> mod.getOperation() == AttributeOperation.MULTIPLY_TOTAL).toArray(AttributeModifier[]::new)) {
